...
document.addEventListener('click', function(event) {
    if (sidebar && !sidebar.contains(event.target) && !btnMenu.contains(event.target)) {
...
function generarStats() {
    ...
    container.innerHTML = Object.keys(counts).map(gen => {
        const porcentaje = (counts[gen] / coleccionSeries.length) * 100;
        return `
            <div style="margin-bottom:15px; padding: 0 15px;">
                <div style="display:flex; justify-content:space-between; font-size:12px; margin-bottom:5px;">
                    <span>${gen}</span>
                    <span>${counts[gen]}</span>
                </div>
